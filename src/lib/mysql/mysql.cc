...
v8::Global<v8::FunctionTemplate> _rest;
v8::Global<v8::FunctionTemplate> _mysqlt;
...
void finalize(v8::Local<v8::Object> obj) {
	if (obj->GetInternalField(1)->IsFalse()) {
		v8::Local<v8::Function> fun = v8::Local<v8::Function>::Cast(obj->Get(JS_CONTEXT,JS_STR("close")));
		fun->Call(obj, 0, NULL);
	}
}

void result_finalize(v8::Local<v8::Object> obj) {
	v8::Local<v8::Function> fun = v8::Local<v8::Function>::Cast(obj->Get(JS_CONTEXT,JS_STR("close")));
	fun->Call(obj, 0, NULL);
}
...
JS_METHOD(_query) {
	...
	int qc = args.This()->Get(JS_CONTEXT,JS_STR("queryCount"))->ToInteger()->Int32Value();
	args.This()->Set(JS_CONTEXT,JS_STR("queryCount"), JS_INT(qc+1));
...
JS_METHOD(_qualify) {
	...
	v8::Local<v8::Value> output = JS_STR(result.c_str());
	args.GetReturnValue().Set(output);
}
 ...
 *                     number.
 */
v8::Local<v8::Value> _mysqltojs(const char* mysql_value, const MYSQL_FIELD& field) {
	if (mysql_value == NULL) {
...
JS_METHOD(_fetcharrays) {
	...
	for (int i = 0; i < y; i++) {
		...
		v8::Local<v8::Array> item = v8::Array::New(JS_ISOLATE, x);
...
JS_METHOD(_fetchobjects) {
	...
	for (int i = 0; i < y; i++) {
		...
		result->Set(JS_INT(i), item);
		for (int j=0; j<x; j++) {
			item->Set(JS_CONTEXT,JS_STR(fields[j].name), _mysqltojs(row[j], fields[j]));
			}
		}
...
JS_METHOD(_storeConnection) {
	...
	v8::Local<v8::Object> inst = v8::Local<v8::Object>::Cast(args[1]);
...
JS_METHOD(_loadConnection) {
	...
	v8::Local<v8::Value> newArgs[1] = {JS_BOOL(true)};
	v8::Local<v8::FunctionTemplate> mysqlt = v8::Local<v8::FunctionTemplate>::New(JS_ISOLATE, _mysqlt);
	v8::Local<v8::Object> inst = mysqlt->GetFunction()->NewInstance(1, newArgs);
...
SHARED_INIT() {
	...
	v8::Local<v8::FunctionTemplate> mysqlt = v8::FunctionTemplate::New(JS_ISOLATE, _mysql);
	mysqlt->SetClassName(JS_STR("MySQL"));

	v8::Local<v8::ObjectTemplate> ot = mysqlt->InstanceTemplate();
	ot->SetInternalFieldCount(2); /* connection, do-not-auto-close */
	
	/**
	 * Static property, useful for stats gathering
	 */
	ot->Set(JS_CONTEXT,JS_STR("queryCount"), JS_INT(0));

	/**
	 * Persistent connection storage
	 */
	mysqlt->Set(JS_CONTEXT,JS_STR("storeConnection"), v8::FunctionTemplate::New(JS_ISOLATE, _storeConnection));
	mysqlt->Set(JS_CONTEXT,JS_STR("loadConnection"), v8::FunctionTemplate::New(JS_ISOLATE, _loadConnection));
		 
	v8::Local<v8::ObjectTemplate> pt = mysqlt->PrototypeTemplate();
	 ...
	 */
	pt->Set(JS_CONTEXT,JS_STR("connect"), v8::FunctionTemplate::New(JS_ISOLATE, _connect));
	pt->Set(JS_CONTEXT,JS_STR("close"), v8::FunctionTemplate::New(JS_ISOLATE, _close));
	pt->Set(JS_CONTEXT,JS_STR("query"), v8::FunctionTemplate::New(JS_ISOLATE, _query));
	pt->Set(JS_CONTEXT,JS_STR("nextResult"), v8::FunctionTemplate::New(JS_ISOLATE, _nextresult));
	pt->Set(JS_CONTEXT,JS_STR("affectedRows"), v8::FunctionTemplate::New(JS_ISOLATE, _affectedrows));
	pt->Set(JS_CONTEXT,JS_STR("escape"), v8::FunctionTemplate::New(JS_ISOLATE, _escape));
	pt->Set(JS_CONTEXT,JS_STR("qualify"), v8::FunctionTemplate::New(JS_ISOLATE, _qualify));
	pt->Set(JS_CONTEXT,JS_STR("insertId"), v8::FunctionTemplate::New(JS_ISOLATE, _insertid));

	v8::Local<v8::FunctionTemplate> rest = v8::FunctionTemplate::New(JS_ISOLATE, _result);
	rest->SetClassName(JS_STR("Result"));
	
	v8::Local<v8::ObjectTemplate> resinst = rest->InstanceTemplate();
	resinst->SetInternalFieldCount(1);
	
	v8::Local<v8::ObjectTemplate> resproto = rest->PrototypeTemplate();
	 ...
	 */
	resproto->Set(JS_CONTEXT,JS_STR("numRows"), v8::FunctionTemplate::New(JS_ISOLATE, _numrows));
	resproto->Set(JS_CONTEXT,JS_STR("numFields"), v8::FunctionTemplate::New(JS_ISOLATE, _numfields));
	resproto->Set(JS_CONTEXT,JS_STR("fetchNames"), v8::FunctionTemplate::New(JS_ISOLATE, _fetchnames));
	resproto->Set(JS_CONTEXT,JS_STR("fetchArrays"), v8::FunctionTemplate::New(JS_ISOLATE, _fetcharrays));
	resproto->Set(JS_CONTEXT,JS_STR("fetchObjects"), v8::FunctionTemplate::New(JS_ISOLATE, _fetchobjects));
	resproto->Set(JS_CONTEXT,JS_STR("close"), v8::FunctionTemplate::New(JS_ISOLATE, _result_close));

	exports->Set(JS_CONTEXT,JS_STR("MySQL"), mysqlt->GetFunction());
}
