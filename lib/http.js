...
ClientRequest.prototype.sendFiles = function (follow) {
	...
	{
		for (let [paramKey, paramValue] of this.post) {
			bodyContent += "\r\n--";
			bodyContent += teaJSBoundary;
			bodyContent += "\r\nContent-Disposition: form-data; name=\"";
			bodyContent += paramKey;
			bodyContent += "\"\r\n\r\n";
			bodyContent += paramValue;
		}

		for (let [paramKey, paramValue] of this.postFiles) // paramValue - filepath
		{
	...
	var post = null;
	if (typeof (this.post) == "object") {
		post = this._serialize(bodyContent);
		if (post.length) {
			post = new Buffer(post, "utf-8");
			this.header({
				"Content-Length": post.length,
				"Content-Type": "multipart/form-data; boundary=" + teaJSBoundary
			});
		}
	} else {
		post = bodyContent;
