...
ClientRequest.send_parallel=function(arr)
{
	...
	for (var i=0;i<arr.length;i++) {
		options[i]=arr[i].send_only(arr[i]._follow);
		options[i]._received=new Buffer(0);
	}
	// Receiving data
	var flag=0;
	while (!flag) {
		...
		for (var i=0;i<options.length;i++) {
			if (!options[i].received) {
				sockets.push(options[i].socket);
				flag=0;
			}
		}
		var sel=Socket.select(sockets,[],[],0);
		for (var i=0;i<options.length;i++) {
			for (var j=0;j<sockets.length;j++) {
				var el=options[i];
				if (el.received) continue;
				if (el.socket!==sockets[j]) continue;
				var part = el.socket.receive(1024);
				if (part && part.length) {
					var tmp = new Buffer(el._received.length + part.length);
					tmp.copyFrom(el._received);
					tmp.copyFrom(part, 0, el._received.length);
					el._received=tmp;
				} else {
					el.received=el._received;
				}
			}
		}
	}
	...
	for (var i=0;i<arr.length;i++) {
		ret[i]=arr[i].recv_only(arr[i]._follow,options[i]);
	}
	return ret;
};

ClientRequest.prototype.send = function (follow) {
	var _options=this.send_only(follow);
	return this.recv_only(follow,_options);
}

ClientRequest.prototype.send_only = function (follow) {
	var dd = new Date();
	var Socket = require("socket").Socket;
	var Buffer = require("binary").Buffer;
...
ClientRequest.prototype.recv_only = function (follow,_options) {
	...
	var received=_options.received;
	...
	try {
		...
		if (!received) received = socket.receive_strict(0);
